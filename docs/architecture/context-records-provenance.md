...
For multi-Source Projects, each candidate is evaluated with its own Source connection. `DEFAULT_BRANCH` applies only to Evidence from that Source on its current default ref; explicit `EXACT_REF` and `ANY_REF` scopes may still win across Sources through the normal precedence rules. Context write then requires the canonical winner to belong to the supplied Evidence Source.
