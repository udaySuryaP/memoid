  ...
  public async loadMaterial(
    ...
    return withSecurityTransaction(this.db, security(context, projectId), async (trx) => {
      const row = (
        ...
        }>`select a.id::text "candidateId",w.context_identity_id::text "contextIdentityId",a.origin_kind origin,a.assertion_payload candidate,encode(a.assertion_hash,'hex') "candidateHash",i.subject_key subject,i.scope_key scope,i.facet_key facet,i.predicate_key predicate,i.version::text "identityVersion",current_state.context_record_id::text "currentRecordId",r.assertion_payload "currentPayload",encode(r.assertion_hash,'hex') "currentHash",exists(select 1 from memoid.candidate_assertions duplicate where duplicate.workspace_id=a.workspace_id and duplicate.project_id=a.project_id and duplicate.id<>a.id and duplicate.assertion_hash=a.assertion_hash) duplicate,(select count(*)::text from memoid.working_context_items wc where wc.workspace_id=a.workspace_id and wc.project_id=a.project_id and wc.context_identity_id=i.id) "workingVersion",coalesce((select max(assignment_version)::text from memoid.source_authority_assignments sa where sa.workspace_id=a.workspace_id and sa.project_id=a.project_id),'0') "authorityVersion",coalesce((select max(ingested_sequence)::text from memoid.source_frontier_states fs where fs.workspace_id=a.workspace_id and fs.project_id=a.project_id),'0') "frontierVersion",greatest(coalesce((select max(occurrence_version) from memoid.conflict_occurrences co where co.workspace_id=a.workspace_id and co.project_id=a.project_id and co.context_identity_id=i.id),0),coalesce((select max(occurrence_version) from memoid.uncertainty_occurrences uo where uo.workspace_id=a.workspace_id and uo.project_id=a.project_id and uo.context_identity_id=i.id),0))::text "integrityVersion",exists(select 1 from memoid.integrity_conflicts c join memoid.conflict_current_states cs on cs.workspace_id=c.workspace_id and cs.project_id=c.project_id and cs.conflict_id=c.id where c.workspace_id=a.workspace_id and c.project_id=a.project_id and c.context_identity_id=i.id and cs.lifecycle_state='ACTIVE') "activeConflict",exists(select 1 from memoid.integrity_uncertainties u join memoid.uncertainty_current_states us on us.workspace_id=u.workspace_id and us.project_id=u.project_id and us.uncertainty_id=u.id where u.workspace_id=a.workspace_id and u.project_id=a.project_id and u.context_identity_id=i.id and us.lifecycle_state='ACTIVE') "activeUncertainty" from memoid.candidate_assertions a join memoid.working_context_items w on w.workspace_id=a.workspace_id and w.project_id=a.project_id and w.candidate_assertion_id=a.id join memoid.context_identities i on i.workspace_id=w.workspace_id and i.project_id=w.project_id and i.id=w.context_identity_id left join memoid.context_identity_current_records current_state on current_state.workspace_id=i.workspace_id and current_state.project_id=i.project_id and current_state.context_identity_id=i.id left join memoid.context_records r on r.workspace_id=current_state.workspace_id and r.project_id=current_state.project_id and r.id=current_state.context_record_id where a.workspace_id=${context.workspaceId}::uuid and a.project_id=${projectId}::uuid and a.id=${candidateAssertionId}::uuid`.execute(
          trx,
        )
      ).rows[0];
      ...
      return {
        comparison: {
          ...
          authorityQualification: sourceDerived ? "MISSING" : "EFFECTIVE",
